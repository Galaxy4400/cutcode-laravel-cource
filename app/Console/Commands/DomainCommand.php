...
class DomainCommand extends Command
{
	...
	public function handle(): int
	{
		...
		File::makeDirectory(base_path("src/Domains/$this->domain"));

		foreach ($this->directories as $dir) {
			File::makeDirectory(base_path("src/Domains/$this->domain/$dir"));
		}
	...
	protected function registerServiceProvider(): void
	{
		...
		if (Str::contains(
			$domainProvider,
			'Domains\\' . $this->domain . '\\Providers\\' . $this->domain . 'ServiceProvider'
		)) {
			return;
		}
	...
	protected function registerRouteRegistrar(): void
	{
		...
		if (Str::contains(
			$routeProvider,
			'App\\Routing\\' . $this->domain . 'Routes'
		)) {
			return;
		}
		...
		file_put_contents($routeProviderPath, preg_replace(
			...
			'protected array $registrars = [' . PHP_EOL
				. $this->tab(2)
				. '\App\Routing\\' . $this->domain . 'Routes::class,' . PHP_EOL
				. $this->tab(2),
			$routeProvider
		));
	}
	...
	{
		file_put_contents(
			app_path("Routing/{$this->domain}Routes.php"),
			str_replace(
				"{{Domain}}",
				$this->domain,
	...
	{
		file_put_contents(
			base_path("src/Domains/$this->domain/Providers/{$name}ServiceProvider.php"),
			str_replace(
				"{{Domain}}",
				$this->domain,
	...
	protected function appRegister(string $prefix): string
	{
		return $this->tab(2) . '$this->app->register(' . PHP_EOL .
			$this->tab(3) . "\\Domains\\$prefix\\Providers\\{$prefix}ServiceProvider::class" . PHP_EOL .
