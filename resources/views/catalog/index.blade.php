	...
	<section>
		...
		<div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-5 mt-8">
			@foreach ($categories as $item)
				@include('catalog.shared.category', ['item' => $item])
			@endforeach
		</div>
	</section>
				...
				<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
					<div class="flex items-center gap-4">
						<div class="flex items-center gap-2">
							<a href="{{ filter_url($category, ['view' => 'grid']) }}" class="@if (is_catalog_view('grid')) pointer-events-none text-pink @else text-white hover:text-pink @endif inline-flex items-center justify-center w-10 h-10 rounded-md bg-card">
							...
							<a href="{{ filter_url($category, ['view' => 'list']) }}" class="@if (is_catalog_view('list')) pointer-events-none text-pink @else text-white hover:text-pink @endif inline-flex items-center justify-center w-10 h-10 rounded-md bg-card">
								<svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 52 52">
					...
					<div x-data="{sort: '{{ filter_url($category, ['sort' => request('sort')]) }}'}" class="flex flex-col sm:flex-row sm:items-center gap-3">
						...
						<form x-ref="sortForm" action="{{ route('catalog', $category) }}">
							<select name="sort" x-model="sort" x-on:change="window.location = sort" class="form-select w-full h-12 px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xxs sm:text-xs shadow-transparent outline-0 transition">
