		...
		<div class="header-inner flex items-center justify-between lg:justify-start">
			...
			<div class="header-menu grow hidden lg:flex items-center ml-8 mr-8 gap-8">
				<form action="{{ route('catalog') }}" class="hidden lg:flex gap-3">
					<input name="search" type="search" value="{{ request('search') }}" class="w-full h-12 px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs shadow-transparent outline-0 transition" placeholder="Поиск..." required>
			...
			<div class="header-actions flex items-center gap-3 md:gap-5">
				...
				<a href="{{ route('cart') }}" class="flex items-center gap-3 text-pink hover:text-white">
					...
					<div class="hidden sm:flex flex-col gap-2">
						@if (!cart()->count())
							<span class="text-body text-xxs leading-none">Корзина<br>пуста</span>
						@else
